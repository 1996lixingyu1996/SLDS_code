...
f_exp1 <- function(seed){
  ...
  parm.tmp$beta$beta13 = c(0, 0, c(-0.1,-0.1,0.1,0.1), rep(0,6), -0.2, -0.2)  # 
  parm.tmp$beta$beta23 = c(0, 0, c(-0.1,-0.1,0.1,0.1), rep(0,6), -0.2, -0.2)  # 
  ...
  ## var12 is tau(x,w)
  for (i in 1:n){
    error = rnorm(n=1,mean=0,sd=0.01)
    if(var[i,1]>0&var[i,2]>0&var[i,13]==1){
      var[i,11] = var[i,1] + var[i,2] + error
      var[i,12] = var[i,1] + var[i,2] + error
    }else{
      var[i,11] = error 
      var[i,12] = error
    }
  ...
  dat_save_path = paste0(".../expe_log_part_varied1000/data_seed_",seed,".csv")
  ...
  for (r in 1:7){
    ...
    dtrain <- xgb.DMatrix(data = xm_1, label = dat_trt1$OS)
    ...
    for (j in 2:5){
      ...
      profiles <- MLearner::tree_fit(Y=dat[,c_name], X=dat[,x_name], seed=1234,maxdepth = 2,min_leaf = 100)
      profiles2 <- MLearner::tree_fit(Y=dat[,c_name], X=dat[,x_name], seed=1234,min_leaf = 180)
      ...
      if(length(profiles$trees)>0){
        for (k in 1:length(profiles$trees)){
          ...
          dat = MLearner::predict_path(profiles$trees[[k]], newdata = dat)
      ...
      if(length(profiles2$trees)>0){
        for (k in 1:length(profiles2$trees)){
          if(has_repeated_vars(profiles2$trees[[k]])){
            next
          }
          dat = MLearner::predict_path(profiles2$trees[[k]], newdata = dat)
          dat$leaf_rs = dat$leaf
  ...
  save_path = paste0(".../expe_log_part_varied1000/seed_",seed,".rds")
...
my_new_folder = ".../expe_log_part_varied1000"
...
folder_path = ".../expe_log_part_varied1000"
...
df_save_path = ".../expe_log_part_varied1000.csv"
